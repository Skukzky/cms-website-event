            ...
            <tr key={index}>
              {Object.keys(data).map(
                (key) =>
                  display.indexOf(key) > -1 && (
                    <td key={key}>
                      {key === 'avatar' ? (
                        <Image height={40} width={40} roundedCircle src={`${config.api_image}/${data[key]}`} />
                      ) : key === 'date' ? (
              ...
              {!actionNotDisplay && (
                <td>
                  ...
                  {editUrl && (
                    <Button variant="success" size={'sm'} onClick={() => navigate(`${editUrl}/${data._id}`)}>
                      Edit
                    </Button>
                  )}
                  {deleteAction && (
                    <Button className={'mx-2'} variant="danger" size={'sm'} onClick={() => deleteAction(data._id)}>
                      Hapus
                    </Button>
                  )}
                </td>
