...
function TalentsPage() {
  ...
  return (
    <Container className="mt-3">
      <ComponentBreadCrumb textSecond={'Talents'} />
      {access.tambah && (
        <div className="mb-3">
          <ComponentButton onClick={() => navigate('/talents/create')}>Tambah</ComponentButton>
        </div>
      )}
      <SearchInput query={talents.keyword} handleChange={(e) => dispatch(setKeyword(e.target.value))} />
      {notif.status && <AlertMessage variant={notif.typeNotif} message={notif.message} />}
      <Table
