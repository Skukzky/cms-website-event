...
function EventsEdit() {
  ...
  const fetchOneCategories = async () => {
    ...
    res.data.data.tickets.forEach((tic) => {
      _temp.push({
        ...
        stock: tic.stock,
        statusTicketCategories: {
          label: tic.statusTicketCategories ? 'Published' : 'Draft',
  ...
  return (
    <Container>
      <ComponentBreadCrumb textSecond={'Events'} urlSecond={'/events'} textThird="Edit" />
      {alert.status && <ComponentAlert variant={alert.type} message={alert.message} />}
      <Form
        form={form}
        isLoading={isLoading}
        lists={lists}
